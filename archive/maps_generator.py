...
def generate_map(n_clicks, metric, year):
    ...
    try:
        ...
        # --- DUMP THE DATA TO TERMINAL (BEFORE COLORING) ---
        print("\n" + "="*70)
        print(f"🔍 DATA DUMP FOR {label} ({year})")
        print("="*70)
        print(f"Total countries with valid color data: {len(df)}")
        print("\n📊 FIRST 10 ROWS (EXACT DATA PASSED TO COLORING):")
        print(df[['country', 'iso_alpha', 'value']].head(10).to_string(index=False))
        print("="*70 + "\n")
